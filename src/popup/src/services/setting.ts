...
const defaultSetting: ISetting[] = [
    {
        ...
        value: true,
        toolTip: "Adds a new tab for incoming comments in the comment section. The \
    ...
    },
    {
        iType: "setting",
        key: "authToken",
        desc: "Permission to read auth token*",
        value: false,
        toolTip: "The extension reads the authentication token (encrypted \
            information about the user) to attach it to a request to webtoon. \
            The token is only required to show what comments a user has \
            already liked in the \"incoming\" tab. For more info, refer to the \
            web store page.",
    },
    {
        iType: "setting",
        ...
        value: true,
        toolTip: "Hides the delete button from all tiles in the dashboard to avoid \
    ...
    },
    {
        ...
        value: false,
        toolTip: "Changes the order of the buttons in the top right corner of the \
            header.",
    },
    {
        iType: "setting",
        ...
        desc: "Hide rating",
        value: false,
        toolTip: "Hides the rating field from the comic overview page. You can not \
    ...
    },
    {
        ...
        value: false,
        toolTip: "Removes last three digits once subscribers count is greater than 1000.",
    },
];
...
async function loadSetting(): Promise<ISetting[]> {
    if (import.meta.env.DEV || !chrome.storage) {
        ...
        if (storedItem) {
            ...
            if (Array.isArray(importedSetting) && importedSetting.every((v) => v.iType === "setting")) {
                return defaultSetting.map((defSetting) => {
                    const impSetting = importedSetting.find((impSetting) => impSetting.key === defSetting.key);
        ...
        return await chrome.storage.sync.get(STORAGE_SETTING_NAME).then((items) => {
            if (STORAGE_SETTING_NAME in items) {
                const importedSetting: ISetting[] = items[STORAGE_SETTING_NAME] satisfies ISetting[];
                return defaultSetting.map((defSetting) => {
                    const impSetting = importedSetting.find((impSetting) => impSetting.key === defSetting.key);
                    if (impSetting) defSetting.value = impSetting.value;
                    return defSetting;
